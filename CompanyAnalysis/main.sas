...
proc sgplot data=sales_data;
  series x=Month y=Revenue / lineattrs=(color=blue thickness=2);
  xaxis label="An" valuesformat=year4.;
  yaxis label="Venit (lei)";
  title "Evolutia veniturilor în timp";
run;

proc sgplot data=employees_data;
  vbar Department / response=Salary stat=mean fillattrs=(color=lightblue);
  title "Salariul mediu pe departament";
  yaxis label="Salariu mediu (lei)";
run;